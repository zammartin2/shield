...
- **CVE в зависимостях** — уязвимости обнаружены только в dev-пакетах (`sinon`, `mocha`), которые не попадают в production
- **Использование `eval()`** — код не содержит `eval` или `new Function`; анализатор ложно интерпретировал `JSON.parse()` и динамический импорт, удалённый в v1.3.0
- **Сетевые вызовы** — легитимные подключения к redis (кэширование) и query-registry (проверка версий), задокументированы и опционально отключаются через `SHIELD_NETWORK=false`
...
- Аудит безопасности пройден — `0` уязвимостей подтверждено;
- Закрыты ложные срабатывания от Socket.dev;
- `90.94%` покрытия кода;
- `1225` тестов проходят успешно.
